...
func (acon AuthController) Profile(c echo.Context) error {
	...
	if r, ok := check.HTTP(token, err, "Get User Profile"); !ok {
		return c.JSON(r.Code, r.Result)
	}

	code, _ := jwt.GetTokenData(token, "code", jwt.ACCESS)
	if r, ok := check.HTTP(code, err, "Get User Profile"); !ok {
		return c.JSON(r.Code, r.Result)
	}
...
// @Success 200 {object} response.MessageData{data=models.Token{}}
// @Failure 417 {object} response.Error{}
// @Failure 500 {object} response.Error{}
